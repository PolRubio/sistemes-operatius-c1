...
int main(){
    ...
    int sock_fd, comm_fd; // file descriptors to be used
    ...
    sock_fd=socket(AF_INET, SOCK_STREAM, 0); // creates a listen connection
    // creates a socket with AF_INET (ip family) and SOCK_STREAM
    // "Data from all devices wishing to connect on this socket will be redirected to sock_fd."
